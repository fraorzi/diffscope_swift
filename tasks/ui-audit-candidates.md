...
### D2 · reader building a partial commit line by line
1. In the combined "all local changes" comparison a line click stages against a *different* comparison than the one on screen — the row's number is carried across unchanged. Before anything is staged the two agree; afterwards the numbers drift and later clicks stage a neighbour, a line already staged, or nothing, while the pane still shows it pending. Direction is inferred from the selected scope rather than from what the clicked line is, so a click can never mean unstage there.
2. A line click that resolves to nothing produces no response at all — no sentence, no flash. The reader cannot tell "already staged" from "missed" from "busy", and the standard recovery is to click again, which is the click that stages something unintended. The keyboard route for the same operation *does* say "nothing to stage here".
3. **File-level and line-level staging write the index by two mechanisms with two different byte results** — one lets git's content filters run, the other writes patch bytes straight in. On a repository normalising CRLF→LF the two disagree permanently: stage every line and the file still reads as modified, the three-state mark stays stuck at partial, and the remaining diff shows every line changed with nothing visibly different.
