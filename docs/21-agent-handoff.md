...
**2026-08-23 — the diff is measured against 4016 of the owner's own changes, and the two shapes that corpus ranked first are fixed. 1990 → 2005 checks, two decisions ([DEC-100](04-decision-log.md), [DEC-101](04-decision-log.md)) and three measurements (M12-A … M12-C).** The owner reported the reflow case again — *a prop was added to `<Image>`, prettier rewrapped it, and the whole element is shown twice* — and asked for it to be fixed **generally**, against their own diffs rather than against one screenshot. So the first thing built was the instrument: `build-corpus.sh` extracts real before/after pairs from thirteen Next.js repositories, `--corpus-survey` runs the shipped pipeline over all 4016 of them and ranks nine named shapes of wrong presentation. M11 was steered by eleven files; this is steered by four thousand changes, and every number below is one run of a switch against its own absence.
...
**Half the report is still open, and it is now precisely stated.** `silent-old-side` and `reflow-insertion` did not move and **cannot** be moved by any mark-level pass: they are about which *lines the unified view prints*, and it still prints a rewrapped element on both sides. The engine has the vocabulary for it now — `hunkLayout` names a region `reflowed` — so what is missing is a `UnifiedBlock` that carries the fact, an old half withheld behind an expander, and the losslessness clause that permits withholding it. That is the next entry, and `tasks/todo.md` carries it with the two smaller costs these entries created.
