...
**2026-08-23 — the diff is measured against 4016 of the owner's own changes, and the three shapes that corpus ranked first are fixed. 1990 → 2013 checks, three decisions ([DEC-100](04-decision-log.md), [DEC-101](04-decision-log.md), [DEC-102](04-decision-log.md)) and four measurements (M12-A … M12-D).** The owner reported the reflow case again — *a prop was added to `<Image>`, prettier rewrapped it, and the whole element is shown twice* — and asked for it to be fixed **generally**, against their own diffs rather than against one screenshot. So the first thing built was the instrument: `build-corpus.sh` extracts real before/after pairs from thirteen Next.js repositories, `--corpus-survey` runs the shipped pipeline over all 4016 of them and ranks nine named shapes of wrong presentation. M11 was steered by eleven files; this is steered by four thousand changes, and every number below is one run of a switch against its own absence.
...
**What is left.** `tasks/todo.md` carries three things these entries created or exposed: the `micro-island` rise the word snap paid for (4766 → 5305 — a widened mark leaves a shorter gap and absorption refuses it), the 27284 junctions **between** words that the confidence floor still draws as two marks, and the 1075 byte-identical lines still printed twice inside blocks that are not rewraps, which is DEC-096's peel territory. The instrument for all three exists now: `Scripts/devtools/build-corpus.sh` and `diffscope-verify --corpus-survey`, which is how every number above was produced and how the next one should be.
