...
**Packaged: `dist/DiffScope-25ef945.zip`**, sha256 `45602c1c8ef5ca8b8af42bfd857d13faf4b7a7278f02b21874d8478869887517`, 2.1 MB, stamped with the commit it was built from. Older zips dropped; `dist/` is git-ignored, so the checksum here is the record. The gate proved what only packaging can: the selftest ran from a temporary directory outside the checkout — the step that catches a build quietly reading from the source tree — and the keyboard walk was not skipped. Give the tester the zip **and** `25-tester-packet.md`; the bundle is unsigned by decision and the packet carries the Gatekeeper step.

**The gate refused once in four, and the log said nothing.** 27 arms, then the log simply ended after `terminal-follow=OK` — no `MISMATCH`, no trace, no exit code, because the arm after it had an **`exit` with no message**. Three further runs passed at 49, 50 and 49. That silent exit now speaks, and `package.sh` prints the exit status and the last arm to report; **the intermittent itself is open**, one in four, in the second-shell arm, on a path this session did not touch. The arm count varies between runs too, so *N arms passed* is not a number to assert on. `tasks/todo.md` step 83.
