...
**2026-09-05 — the diff-engine audit is closed: false lines 17.5% → 5.0%, and the corpus is validated for the first time. 2221 → 2251 checks, 41 → 42 selftest arms.**

Same method as the UI audit and it held: 25 isolated generators with no code access produced **200 candidates**, five verifiers labelled every one, and **35 of the 165 non-duplicates were wrong** — the refutations are in `tasks/diff-audit-verified.md` and are worth reading before proposing anything in this area again.

**Seven repairs, each with a reproduction, a before-and-after over 4016 real changes, and a negative control that was watched failing.**

- **[DEC-122](04-decision-log.md) is the one that moved the most.** `reconcile` opens `guard applied else { return segments }`, and `applied` is false whenever the canonical diff runs out of budget — so on those pairs the anchor and gap marks shipped **unclipped**. Thirty-nine pairs of 4016, one percent, were producing **two thirds of the corpus's false lines**. The mask is now DEC-105's line anchoring, whose guarantee covers every byte that differs.
- **[DEC-118](04-decision-log.md)**: the same 39 pairs had `changeStops` return `[]`, and `unifiedBlocks` opens `guard !stops.isEmpty` — so the launch layout showed **no blocks at all** and ⌘↓ was dead, while the notice bar said only *coverage not verified*.
- **[DEC-123](04-decision-log.md)**: the syntax snap was the entire line cost of the widening stack. Bounded by the line as well as by its budget it reaches the false-line count of turning the pass off while keeping 73% of the marks it merges.
- **[DEC-117](04-decision-log.md)** answers the owner's first report — a moving line break no longer swallows the code beside it. **[DEC-119](04-decision-log.md)** answers the second — a closing token on its own line no longer prints the whole element twice. **[DEC-120](04-decision-log.md)** stops four passes turning two kinds of formatting into no classification at all. **[DEC-124](04-decision-log.md)** re-measures `matchConsumeFloor` on the corpus rather than on the eleven files its comment cited.

**The corpus surveys had never called `validate`.** Four thousand real files went through the shipped pipeline on every run since M11 with no invariant asked of them, while the check suite validated a few dozen fixtures. Asked for the first time, `corpus-styles` reported **eleven pairs violating INV-2** — reproduced at `500e568`, so shipping since DEC-105. The cause was a disagreement of *parameters*, not of implementation: the fallback built its model at a tenth of the canonical budget and `validate` checked it at the full one, so a model built from line anchors was measured against an alignment it had never been shown. [DEC-125](04-decision-log.md) equalises them, because INV-2 is stated against *the* canonical minimal diff and not against a family indexed by budget.

**Open, and the owner's to decide: the third report.** A wrapper added or removed still reads as a rewrite. The engine is not at fault — it classifies the reindent correctly, six of eight added lines on the real corpus pair carrying only formatting marks — and [DEC-083](04-decision-log.md) draws formatting identically to a real change. Two routes were tried and both are closed by measurement: a third tint does not fit the luminance ladder (1.267 light, 1.370 dark, against a 1.20 rule), and the formatting-group-in-unified route could not be demonstrated and was reverted. **DEC-083's own revisit trigger — *reopen if a reader ever asks why is this line marked* — has fired.**

**51 NEEDS-MEASUREMENT items** are recorded per area with the command that would settle each. Most need instrumentation that does not exist; they are a milestone, not a loose end.

**2026-08-28 — a stylesheet's marks stopped all being drawn uncertain ([DEC-116](04-decision-log.md), M12-K). 2078 → 2086 checks.** The last of the three items M12-H left open, and the one that turned out to be a switch. `.css`, `.scss`, `.json` and `.md` take the fallback path, and every mark it made arrived at `confidence: 0` — so against a floor of 0.8 the contract's `uncertain` flag was true for **99.3% of marks and 100.0% of presented bytes** across 364 such pairs. The dotted outline and the dashed underline that mean *this alignment could not be confirmed* were drawn over everything, which is the same as not drawing them. **Nothing could have caught it**: the flag is computed correctly from the number it is given, and the number was a constant nobody had revisited since the fallback path had nothing but `wholeFilePartition` to offer.

**The number now follows how the range was obtained**, in the vocabulary the engine already uses. The canonical byte diff — the alignment INV-2 states the structural path *against* — takes **0.8**, the same value an ordinary structural change gets, because it is the same claim by the same means. DEC-105's line-anchored route takes **0.6**, the value already meaning *the byte diff and the alignment disagree*: its guarantee is about what it leaves unmarked, and the boundaries of what it marks are wider than minimal. A whole-file answer stays at 0, and an unchanged whole file becomes 1, which it always was. What says *this file was not parsed* is the `.fallback` label and the notice bar, and both are untouched.

**The rest of the table is the finding: the flag was a switch nobody read as one.** `absorbIslands` and `coalesceAdjacent` both refuse to merge across the floor, deliberately — and with every mark at 0 and every unchanged byte at 1, *every* junction on this path crossed it. DEC-094's absorption, DEC-100's word snap and DEC-107's word merge were switched off in a whole family of files. **`split-mark` 54 → 0**, `micro-island` 329 → 126, `mark-confetti` 21 → 10, marks 4134 → 3868, for 0.09% more presented bytes — and **`false` and `missed` lines identical to the line**, because nothing here changes which bytes are marked. The control is the 4016-pair TypeScript corpus: **identical line for line**, measured rather than argued from which function was touched.

**What remains is honest rather than open**: the 332 uncertain marks left are the line-anchored route, 60.8% of presented bytes because the files reaching it are the big ones. If that share grows, *uncertain* starts meaning *large* rather than *guessed*, which is the same defect one level up — that is the revisit trigger. `fallback.png` is the picture: `200ms` → `240ms` and `hover` → `focus`, one clean mark each, under a chip still reading `plain text`.

---

**2026-08-28 — the two-pane layout stopped printing a rewrapped element twice ([DEC-115](04-decision-log.md)).** Candidate 2 of the three the corpus ranked on 2026-08-23, and the shortest of them: the fact was already in the model, already checked, and already drawn — by one of its two readers. `withheldOld` says which old lines a rewrap makes redundant (DEC-102, made line-by-line by DEC-108), unified has held them back since M12-D, and the left pane went on drawing the whole rewrapped element with nothing beside it saying the same code was one pane to the right. **Which is the layout the owner was reading when they reported the case.**

**A withheld range is a fold**, because it is not a new act: content out of sight, its count stated, one click or one ⌘E from coming back — DEC-017's terms exactly. Three rules follow from *what* it holds back. The right pane draws none, because it holds the bytes the marker points at. The edges are solid rather than dashed, because what is behind it is not byte-equal to anything in this pane. And unified draws none, because it withholds a rewrap while *composing* its document, so a second marker would be a second expander over the same lines.

**One fact, one expansion state**: a rewrap fold is collapsed while its *block* is, not while its own index is, so ⌘E, the marker, the unified hunk header and a jump landing inside one are four routes into a single answer. The two layouts pay differently to open one and the difference is in the code with its reason — unified rebuilds its document, the two-pane layout refreshes decorations rather than throwing away the reader's scroll position.

**None of this is in the model, which is why the proof is a picture.** `reflowed.png`: three old lines behind one marker that says so, nine new ones beside it, and the footer counting them again where a reader who has scrolled past the marker can still see it. No assertion about the model could have found this, because the model was right. 2078 checks, 35 selftest arms.

---

**2026-08-23 — the diff is measured against 4016 of the owner's own changes, and the shapes that corpus ranked first are fixed. 1990 → 2018 checks, four decisions ([DEC-100](04-decision-log.md) … [DEC-103](04-decision-log.md)) and five measurements (M12-A … M12-E).** The owner reported the reflow case again — *a prop was added to `<Image>`, prettier rewrapped it, and the whole element is shown twice* — and asked for it to be fixed **generally**, against their own diffs rather than against one screenshot. So the first thing built was the instrument: `build-corpus.sh` extracts real before/after pairs from thirteen Next.js repositories, `--corpus-survey` runs the shipped pipeline over all 4016 of them and ranks nine named shapes of wrong presentation. M11 was steered by eleven files; this is steered by four thousand changes, and every number below is one run of a switch against its own absence.

**The ranking is the finding.** By instances the top three are all about marks rather than alignment — `split-mark` 30942, `whitespace-only-mark` 13090, `shredded-word` 6723 — and by share of pairs the top two are the reported case, `silent-old-side` and `reflow-insertion`, in **nearly half of all changes**. DEC-100 takes the first pair: a mark finishes the word it cut (a Tailwind class, an identifier), and two marks that meet inside a word are drawn as one. DEC-101 takes the third: a mark over whitespace a rewrap moved says `whitespace`, so a rewrapped element costs one loud mark instead of twelve. **`shredded-word` 6723 → 682, marks −7.1%, quiet bytes +47%, for 1.6% more presented bytes — and `false` and `missed` lines do not move at all**, which is the property both passes rest on rather than a coincidence.

**The suite refused two drafts of DEC-101 and was right both times.** *Any mark made only of whitespace is formatting* quietened four marks of a **reorder** — `prop-reordering` failed on the first run — and the obvious guard, keyed on the `reordering` classification, changed nothing because that fixture produces no classified segment at all. **Measure the control before believing the check**, twice in one pass. The rule that shipped asks the token sequence, over the region the hunks jointly cover, because a reorder is only visible at the scale of the thing reordered.

**And then the other half was built: [DEC-102](04-decision-log.md), M12-D.** `UnifiedBlock` carries `reflowed` — every token of the old half is on the new half, in order, tested in **one direction only** so a removal is never withheld — and the unified layout holds that half back behind an expander in the hunk header that says how many lines are behind it. Over the same 4016 changes: **`silent-old-side` 3986 → 202, `reflow-insertion` 3795 → 0, `duplicated-line` 2320 → 1075**, with 3910 withheld blocks in 46.6% of pairs — and `marks`, `presented bytes`, `false` and `missed` identical to the byte, because this changes the layout and not the model. The 202 that remain are blocks where something really was removed beside the rewrap, and those must print both sides.

**[DEC-103](04-decision-log.md) is the entry the instrument found rather than the owner.** M12-C recorded a cost — `micro-island` rose 4766 → 5305 when the word snap shipped — and the obvious suspect was DEC-094's floor. Sweeping it over the corpus moved the marks by 0.5%, so the survey was made to say *why* each island survived, against absorption's own four conditions: **1507 of 1757 were refused by no condition at all**, which is the signature of a pass that never saw them. It runs before the wideners, on purpose, and cannot see the gaps they create. Absorption now runs a **second** time after them: `micro-island` **5305 → 1347**, marks **75873 → 70916**. Cumulatively across the four entries: **marks 81665 → 70916, −13.2%, for 2.0% more presented bytes, with `false` and `missed` lines untouched throughout.**

**What is left, and one thing deliberately not done.** `tasks/todo.md` carries the 1075 byte-identical lines still printed twice inside blocks that are not rewraps — DEC-096's peel refuses them because a stop covers them, so the answer is a finer stop rather than a wider peel. And the 27423 junctions where two marks stay two because they cross the confidence floor are **left alone on the measurement**: the same run says uncertain marks are 7.9% of marks and 3.0% of presented bytes, which is rare enough that the flag still means something. The instrument for all of it is `Scripts/devtools/build-corpus.sh` and `diffscope-verify --corpus-survey`, which is how every number above was produced and how the next one should be.
...
**Three of the five reported cases are closed.** `'base' | '⟦compact' | ⟧⟦~'⟧wide'` is now `'base' | ⟦'compact' | ⟧'wide'` — three marks to one, and the stray apostrophe gone at its source rather than masked. `}: ImageTextProps) {` carries no mark. A 589-line stylesheet with a ten-line change marks ten lines instead of 589, and the hairline that boxed it now marks an unparsed *region*, which is what its own comment always said it was for. Over the corpus, against `bb6d9ae`: marks **185 → 160**, wrong lines **24 → 22**, presented bytes **5542 → 5570**. The last of those went **up**, by 0.5%, and that is not a regression to explain away — it is DEC-094's stated price, paid to turn confetti into solid blocks, and DEC-047 paid 4.4% for less.

**Two are not, and they are one thing.** `⟦~s⟧⟦rc⟧` over an unchanged `src`, and 32 lines the unified view still prints twice. Both sit in reflowed JSX, where the old bytes are a subsequence of the new and a minimal alignment legitimately puts every changed byte on one side. **Four separate measurements now point at it and no boundary rule reaches any of them** (M11-B, D, F, G): absorption and the peel are as far as monotone widening goes, and the alignment is already minimal. The answer is a presentation that shows a **substitution on both sides**, which is not an alignment change at all. It is the largest open item in `tasks/todo.md`.

**Two findings worth carrying, both from checks rather than from reading.** DEC-096's containment property failed on its first run — a peel that excluded line terminators dropped a stop out of every block on `moved-function` — and it could only be asked at all because that entry moved `unifiedBlocks` out of `main.js` into the engine, closing the last place where the layout decided *what is shown* without being checkable. And T-11 failed on 192 disagreements when absorption first ran, because the two sides are widened independently and DEC-038 wants a move byte-identical on both; a `moved` flank is refused outright now, since a rule that holds only when the sides happen to agree is not a rule.

**One knob was designed, proved unreachable, and removed before shipping.** DEC-094 was going to cap absorbed bytes per run; the relative rule beside it already bounds the total strictly below the run's own changed bytes, so the cap could never turn. This repository has three recorded defects that were exactly that, and the distinction against DEC-096's peel — rare but reachable — is written down.
