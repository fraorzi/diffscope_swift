...
**The chrome is one surface** ([DEC-088](04-decision-log.md), reversing DEC-080). `--ds-chrome`, `--ds-panel-repos` and `--ds-panel-files` hold the **same value**, and the only step that survives is the code against the chrome around it — at least 1.10:1, in both appearances. DEC-080 pulled the four apart into a ladder because they were nineteen values out of 255 apart and nobody could see where one region ended and the next began; the owner's answer to the same complaint is the other one, and the seams do the separating. A design may pull them apart again — the three names are still three names — but *nearly the same* is what the check rejects, because three neutrals a hundredth apart read as a mistake rather than as a decision.
