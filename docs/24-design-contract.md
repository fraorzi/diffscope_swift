...
| `ds-note` | A note after the line: which move a line belongs to (`M1`), that a difference is invisible, that a change is formatting or a reordering, that an alignment is uncertain, and `inserted` / `removed` on a one-sided block | No — **annotation only.** Every note repeats something the line already carries in shape (the sign column, the underline, the texture), so it may be restyled or dropped. **It sits after the code rather than at the pane's right edge**, which the design draws: right-aligning it needs `position: relative` on `.cm-line`, and that breaks CodeMirror's line measurement — the number columns drift out of step with the code, which is the one thing in this pane that may not happen. It may never be the *sole* carrier of any of them, and it may never say something the engine did not: `label`, `classification`, `group`, `disclosure` and `link` are the whole vocabulary, and the adopted design's *wrapper removed* is prose with nothing behind it |
