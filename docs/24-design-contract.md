...
| `ds-fold-reflow` | The old half of a rewrap, held back in the **two-pane** layout ([DEC-115](04-decision-log.md)): every token of those lines is on the new side, in order, so the left pane would otherwise draw the same code the right pane already does. Solid edges, because what is behind it is not byte-equal to anything in this pane — and an edge on the right, because *the code is that way* is the half of the sentence that makes withholding it honest | **Yes** — the count and the sentence both. It is the same act `ds-fold` performs and it is permitted on the same terms: stated, and one click or one ⌘E from coming back |
...
| `#file-header`, `#file-path`, `#file-name` | Which file this is, above everything said about it | **Yes.** The path is what tells two files of the same name apart, and DEC-058 has already paid three times for putting a displayed fact in a tooltip. Since [DEC-088](04-decision-log.md) its height is `--ds-pane-header-height`, the same number the two lists' headers are: it is the third of three headers across the window and it was the one laid out separately, so its seam sat five points above theirs |
| `#notices` | The bar carrying every notice — INV-4 made visible | **Yes, while it holds one.** [DEC-088](04-decision-log.md) collapses it when it is empty, which on a normal file is every file the reader opens — a bar keeping its padding and its seam to report that there is nothing to report. INV-4 is a promise about a notice that *exists*, and the live audit now puts a chip in before it measures, which is the stronger question |
| `#showing` | **Removed by [DEC-088](04-decision-log.md).** `SHOWING HEAD ↔ working tree · unified`, a band of its own on every file. It was added because the comparison was stated only in the chrome, far from the pane being read — and the chrome states it in the status line *and* in the title band, so this was a third copy of a sentence that does not change until the reader changes the scope. The layout word it also carried names the thing the reader just pressed ⌘E for. `comparisonDescription` and `baseSummary` are untouched; the sentence they compose is still checked, and still drawn in the chrome | Gone |
...
| `ds-hunk-reflowed` | The same header over a block whose old half is **withheld**: every token of it is on screen in the new half and only the wrapping differs (DEC-102). Dashed edges and a pointer, because it is an expander; the header itself says how many lines are behind it | No — it withholds nothing the reader cannot bring back with one click, and the count is stated |
...
| `MarkButton` | A borderless button whose glyph is a **path** rather than a character ([DEC-091](04-decision-log.md)). The collapse chevrons wear it | **Yes as an affordance.** They were `«` and `»` — *guillemets*, quotation marks in French and Polish typography, used as arrows beside a chevron this project draws itself. A mark set in a font carries that font's stroke weight, its optical centre and its side bearings, and none of the three belongs to the control. The `title` stays: it is what VoiceOver reads, what the arms name the control by, and what the mark reads back to decide which way to point — it simply stops being the picture |
| `TerminalButton` | `>_` in the status line, which opens and closes the terminal drawer ([DEC-090](04-decision-log.md)) | **Yes as an affordance.** It was the only pane in the window with no control of its own — the two lists have chevrons and the diff has the lens — so the drawer existed only for a reader who already knew ⌃`. It sits in the status line because that is the edge the drawer comes out of. Its words are `KeyboardMap`'s, so a button and a menu item cannot describe one command two ways; the mark is **drawn**, for the reason `ChevronButton` below records; and open, it wears the raised surface the chosen pill wears, so the state is a shape rather than only an ink |
...
| `IndentGuides` | The vertical rules down the changed-file tree, one per level of nesting ([DEC-099](04-decision-log.md)) | **Yes.** Indentation on its own leaves a file eight levels deep floating in white space with nothing tying it to its parent — which is the failure a tree is meant to fix rather than introduce. The rule is the window's own hairline at the width every other seam uses, so the tree reads as part of the chrome and not as decoration inside it |
| `CheckButton` | The box beside a path that says whether the file is in the next commit ([DEC-092](04-decision-log.md)). **Three states**, drawn as a path: empty, a tick, and a **dash** for a file only part of which is staged | **Yes, and the third state is the rule.** *Some of this file* is a different fact from *this file*, and a mark that differed only in size would have to be compared against a neighbour to be read. The state is read back from the index on every reload rather than remembered, so the box cannot disagree with git |
| `CommitBox` | GitHub Desktop's commit box, pinned under the changed-file list: summary, description, the amend switch, and one wide button reading **Commit to `<branch>`** ([DEC-092](04-decision-log.md)) | **Yes for the branch in the button.** *Commit* on its own does not say where it lands, and a reader who has just switched branches is asking exactly that. ⌘⏎ commits while the box has focus and opens the editor everywhere else, which is GitHub Desktop's own rule; ⇧⌘⏎ commits from anywhere |
| `OperationBanner` | The band that says the repository is mid-merge, mid-rebase, mid-cherry-pick, mid-bisect or detached — **with the verbs on it** ([DEC-092](04-decision-log.md)) | **Yes, and it may not look normal.** Its surface is the one band in the window that is neither chrome nor panel: a repository in a state the reader did not choose, with no way out of it on screen, is the most confusing thing a Git interface can produce. The verbs are rebuilt from the state rather than hidden, so a stale button cannot offer an action that no longer applies |
| `SyncButton` | GitHub Desktop's one sync control with its three states — `Fetch origin`, `Pull origin N`, `Push origin N`, and `Publish branch` for a branch with no upstream | **Yes.** One button that changes its own label, never two buttons that can disagree about which direction the repository is out of date in |
...
**The chrome is one surface, and the status line is the one exception** ([DEC-088](04-decision-log.md), reversing DEC-080). `--ds-chrome`, `--ds-panel-repos` and `--ds-panel-files` hold the **same value**, and the only step that survives is the code against the chrome around it — at least 1.10:1, in both appearances. `--ds-status-bar` sits **darker than the panels** in both appearances, because that band reports *on* the window rather than holding any part of what is being read. Its check is an **ordering rather than a ratio**: below the panels in dark there is almost nothing left before the code's black, so what is asserted is which way, and how far is printed. The hairline at the bar's top edge separates it; the surface reinforces the seam. DEC-080 pulled the four apart into a ladder because they were nineteen values out of 255 apart and nobody could see where one region ended and the next began; the owner's answer to the same complaint is the other one, and the seams do the separating. A design may pull them apart again — the three names are still three names — but *nearly the same* is what the check rejects, because three neutrals a hundredth apart read as a mistake rather than as a decision.

**The lists' scrollbar is drawn only while the reader is scrolling** ([DEC-088](04-decision-log.md)). It is an overlay scroller whatever *Show scroll bars* is set to, with a knob narrower than AppKit's and **no slot behind it**. This is a second exception to *quietened, never removed*, and it is the same one `#track` earned: a track painted for as long as a list is longer than its pane is a stripe down the side of every row, and there is nothing in it to quieten.
