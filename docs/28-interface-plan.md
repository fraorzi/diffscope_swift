...
**5. The title band becomes the system's material** — **landed.** `NSVisualEffectView`, `.headerView`, `.behindWindow`, behind a transparent titlebar, with the window no longer opaque so what is behind it can come through. Every other surface in the chrome paints its own colour, so the band and the titlebar above it are the only see-through parts.

**DEC-083's ban was narrowed, not dropped, and the narrowing is the interesting part.** It read *nothing imitates the material where the system has none* and banned the class outright. That is right where it was aimed — vibrancy standing in for `NSGlassEffectView` **on a control**, on a system that does not have one — and too wide as written: a window band is not a control, and the system's blur cannot be an imitation of the system's blur. The check names the **surface** now: no control file may reach for vibrancy, the window file may, **once**, and the check says which line and refuses a second.
